...
default_multiplicative_sparse_experiment_config <- function(n_train = 640L,
                                                            d = 5L,
                                                            fit_it = 200L,
                                                            target_k = 1L,
                                                            model = 1L,
  ...
  list(
    ...
    scenario = binning_scenario("multiplicative_sparse", n_train, d, rho, model)
  )
}

default_multiplicative_sparse_binning_specs <- function() {
  list(
...
multiplicative_fit_sbf_obj <- function(data, bandwidth, fit_it, kernel) {
  start <- proc.time()[[3L]]
  fit <- sbf_fit(
    data = data,
    bandwidth = bandwidth,
...
multiplicative_fit_binning_obj <- function(data, bandwidth, fit_it, kernel, spec) {
  start <- proc.time()[[3L]]
  fit <- sbf_fit_binning(
    data = data,
    bandwidth = bandwidth,
...
run_multiplicative_sparse_binning_experiment <- function(n_rep = 10L,
                                                         base_seed = 2026050211L,
                                                         kernel = "epanechnikov",
                                                         include_sbf = TRUE,
  ...
  for (run in seq_len(as.integer(n_rep))) {
    ...
    fits <- list()
    if (include_sbf) {
      fits[[length(fits) + 1L]] <- multiplicative_fit_sbf_obj(data, config$bandwidth, config$fit_it, kernel)
    }
    binned_fits <- vector("list", length(binning_specs))
    for (i in binning_rotated_spec_indices(length(binning_specs), run)) {
      binned_fits[[i]] <- multiplicative_fit_binning_obj(
        data, config$bandwidth, config$fit_it, kernel, binning_specs[[i]]
      )
    }
    fits <- c(fits, binned_fits)
  ...
  run_dir <- if (is.null(out_dir)) {
    binning_default_run_dir("multiplicative_sparse", include_sbf, config, n_rep, base_seed)
  } else {
    out_dir
  }
...
if (sys.nframe() == 0L) {
  ...
  cat("Multiplicative binning experiment complete.\n")
