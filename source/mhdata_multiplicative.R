...
mhrate <- function(Z,model=1,violate.cox=TRUE){
    ...
    if (model==1)  surv.function <- function(t){ pexp(t,rate=exp(top),lower.tail = FALSE)} # this is computing the survival prob S(t), only valid under the case that t is exponentially distributed
    ...
    # true_par<-exp(true_par)
     return(list(top=top, surv.function=surv.function, phi = phi))
}
